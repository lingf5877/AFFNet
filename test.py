...
def test(data,
    ...
    if not training:
        ...
        if task == "test":
            ...
            dataloader = create_dataloader_rgb_ir(test_path_rgb, test_path_ir, imgsz, batch_size, gs, opt, pad=0.5, rect=True, prefix=colorstr(f'{task}: '))[0]
        else:
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            out, _, train_out, fea = model(img_rgb, img_ir, augment=augment)  # inference and training outputs
            t0 += time_synchronized() - t

            # Compute loss
            if compute_loss:
                # loss += compute_loss([x.float() for x in train_out], targets)[1][:4]  # box, obj, cls
                loss += compute_loss([x.float() for x in train_out], targets, fea, 0, 1)[1][:5]  # box, obj, cls
        ...
        if plots and batch_i < 3:
            ...
            f1 = save_dir / f'test_batch{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(img_rgb, targets, paths, f1), daemon=True).start()
            f2 = save_dir / f'test_batch{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(img_rgb, output_to_target(out), paths, f2), daemon=True).start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/data2/home/wj4/jw_stu/drone/AFFNet/data/model_data/test1/exp25/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='/data2/home/wj4/jw_stu/drone/AFFNET/config/dronevehicle.yaml', help='*.data path')
    ...
    p = "/data2/home/wj4/jw_stu/drone/AFFNet/data/dronevehicle/test/labels"
