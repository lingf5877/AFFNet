...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.CFls = ContrastFeatureLossMutualInfo() 
        for k in 'na', 'nc', 'nl', 'anchors':
            setattr(self, k, getattr(det, k))

    # def __call__(self, p, targets):  # predictions, targets, model
    def __call__(self, p, targets, fea, epoch, total_epoch):  # predictions, targets, model
        ...
        cfls = torch.zeros(1, device=device) 
        ...
        lcls *= self.hyp['cls']
        lrk *= 0.1
        cfls_weight = max(0, 1 - epoch / (total_epoch * self.attenuation_ratio)) * 0.1  # 线性衰减
        print("cfls_weight:", cfls_weight, " epoch:", epoch, " end_epoch:", total_epoch * self.attenuation_ratio)
        cfls *= cfls_weight
        # print("lbox", lbox)
        # print("lobj", lobj)
        # print("lcls", lcls)
        ...
        loss = lbox + lobj + lcls + lrk + cfls
        return loss * bs, torch.cat((lbox, lobj, lcls, lrk, cfls)).detach()
