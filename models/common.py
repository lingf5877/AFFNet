...
class CrossAttention(nn.Module): #CFE
...
class CrossTransformerBlock(nn.Module):  # 疑似ICFE
...
class TestModule(nn.Module):
    ...
    def forward(self, X, Y):
        ...
        前向传播。
        参数:
            X: 指导融合的权重图，形状为 (batch_size, 1, height, width) * 2
            Y: [new_rgb_fea, new_ir_fea]，包含两个模态的特征图。
                - new_rgb_fea: RGB 模态特征图，形状为 (batch_size, channels, height, width)。
                - new_ir_fea: IR 模态特征图，形状为 (batch_size, channels, height, width)。
        返回:
            fuse_fea: 融合后的特征图，形状为 (batch_size, channels, height, width)。
        """
        # 分离模态特征图
        fuse_map_rgb, fuse_map_ir = X  #  RGB 和 IR 指导融合特征图
        rgb_fea, ir_fea = Y  #  RGB 和 IR 模态特征图

        # 权重图与尺寸对齐
        _, _, H, W = ir_fea.shape

        fuse_map_rgb = F.interpolate(fuse_map_rgb, size=(H, W), mode='nearest')
        fuse_map_ir = F.interpolate(fuse_map_ir, size=(H, W), mode='nearest')

        # save_dir = '/disk1/tzy/drone/ICAFusion/feature_save/fuse_map/'
        # block = [fuse_map_rgb, fuse_map_ir]
        # black_name = ['fuse_map_rgb', 'fuse_map_ir']
        # plt.figure()
        # for i in range(len(block)):
        #     print("i", i) 
        #     print("block", block[i].shape) 
        ...
        #     plt.imshow(feature)
        # timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        # print('fuse_{}x{}_{}.png'.format(shape, shape, timestamp))
        # print('fuse_map_rgb', fuse_map_rgb)
        # print('fuse_map_ir', fuse_map_ir)
        # plt.savefig(save_dir + 'fuse_{}x{}_{}.png'.format(shape, shape, timestamp), dpi=300)
        # plt.close()

        # 进行融合特征
        fuse_fea = (rgb_fea * fuse_map_rgb + ir_fea * fuse_map_ir).half()
        # print("fuse", fuse_fea.dtype)
        return fuse_fea
...
class TransformerFusionBlock(nn.Module):
    ...
    def forward(self, x):
        # print("x0", x[0].shape, x[0]) 
        # print("x1", x[1].shape, x[1]) 
        rgb_fea = x[0]
        ir_fea = x[1]
        fuse_map = x[2]
        ...
        rgb_fea_flat, ir_fea_flat = self.crosstransformer([rgb_fea_flat, ir_fea_flat]) #ICFE
        ...
        new_rgb_fea = rgb_fea_CFE + rgb_fea  # F^R
        ...
        new_ir_fea = ir_fea_CFE + ir_fea  #F^T

        new_fea = self.testmodule(fuse_map, [new_rgb_fea, new_ir_fea]) # 使用融合权重指导融合
        # new_fea = self.concat([new_rgb_fea, new_ir_fea])
        # new_fea = self.conv1x1_out(new_fea)

        # # ------------------------- feature visulization -----------------------#
        # save_dir = '/disk1/tzy/drone/ICAFusion/feature_save/fea/'
        # print("rgb_fea", rgb_fea.shape) 
        # fea_rgb = torch.mean(rgb_fea, dim=1)
        # print("fea_rgb", fea_rgb.shape) 
        ...
        #     plt.imshow(feature)
        # timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        # plt.savefig(save_dir + 'fea_{}x{}_{}.png'.format(h, w, timestamp), dpi=300)
        # # -----------------------------------------------------------------------------#
