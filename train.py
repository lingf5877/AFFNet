...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    with open(opt.data) as f:
        data_dict = yaml.safe_load(f)  # data dict（读取数据集配置信息）
    ...
    if pretrained:
        ...
        for key in list(state_dict.keys()):
            new_state_dict[key[:6] + str(int(key[6])+10) + key[7:]] = state_dict[key]   # 进行更名（）
    ...
    labels_path = data_dict['path'] + '/val/labels'   # 用于判断验证时的标签是否有效
    ...
    accumulate = max(round(nbs / total_batch_size), 1)  # accumulate loss before optimizing（梯度累计）
    hyp['weight_decay'] *= total_batch_size * accumulate / nbs  # scale weight_decay （权重衰减调整）
    logger.info(f"Scaled weight_decay = {hyp['weight_decay']}")

    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups （参数组）
    ...
    # Image sizes
    gs = max(int(model.stride.max()), 32)  # grid size (max stride)（依据最大下采样步幅计算网格大小）
    nl = model.model[-1].nl  # number of detection layers (used for scaling hyp['obj'])
    # print("nl", nl)
    imgsz, imgsz_test = [check_img_size(x, gs) for x in opt.img_size]  # verify imgsz are gs-multiples （确保输入图像与网格兼容）
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if flage_visual:
                ...
                for num in range(batch_size):
                    ...
                    image.save('feature_save/img/example_%s_%s_%s_color.jpg'%(str(epoch), str(i), str(num)))
                    image = imgs[num, 3:, :, :].cpu().clone()  # clone the tensor
                    image = image.squeeze(0)  # remove the fake batch dimension
                    image = unloader(image)
                    image.save('feature_save/img/example_%s_%s_%s_ir.jpg'%(str(epoch), str(i), str(num)))
            ...
            with amp.autocast(enabled=cuda):
                ...
                # pred = model(imgs_rgb, imgs_ir)  # forward(前向传播)
                pred, fea = model(imgs_rgb, imgs_ir)  # forward(前向传播)
                # loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size（计算损失，总损失和各损失项）
                loss, loss_items = compute_loss(pred, targets.to(device), fea, epoch, epochs)  # loss scaled by batch_size（计算损失，总损失和各损失项）
                if rank != -1:
        ...
        # Scheduler
        lr = [x['lr'] for x in optimizer.param_groups]  # for tensorboard （更新学习率）
        scheduler.step()

        # DDP process 0 or single-GPU（使用test脚本计算Map）
        if rank in [-1, 0]:
            # mAP
            ...
            #         ]
            keys = ['train/box_loss', 'train/obj_loss', 'train/cls_loss', 'train/rank_loss','train/cf_loss',  # train loss
                    'TP', 'FP', 'FN', 'F1', 'metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95',  # metrics
                    'val/box_loss', 'val/obj_loss', 'val/cls_loss', 'val/rank_loss','val/cf_loss',  # val loss
                    'x/lr0', 'x/lr1', 'x/lr2',  # learning rate
                    'MR_all', 'MR_day', 'MR_night', 'MR_near', 'MR_medium', 'MR_far', 'MR_none', 'MR_partial', 'MR_heavy', 'Recall_all'  # MR
                    ]
